...
import pageTransitions from "../../../TransitionManager/Transitions";
...
import { ChainedAnimation } from "../../../TransitionManager/Transitions/types";
import { useAnimationController } from "../../../TransitionManager/useSequentialAnimator";
...
const PageIconAnimated: React.FC<Props> = ({
  ...
  const domRect = React.useMemo(() => {
    return ctnRef.current?.getBoundingClientRect() || ({} as DOMRect);
  }, [ctnRef.current]);
  ...
  const [controllers, transitionManager] = useAnimationController({
    ...
    animationDependencies: {
      name: "random",
      container: ctnRef.current as HTMLElement,
    },
    emitter: (message: string, payload: any) => console.log(message),
  });

  React.useEffect(() => {
    const render = () => {
      transitionManager.direction = "backward";
    }
  ...
  // console.log(controllers, "The current controller")
  ...
  return (
    ...
    >
      <PageIcon className={`${className} h-full opacity-0 w-min`} />
